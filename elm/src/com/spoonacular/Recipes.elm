...
module Api.Request.Recipes exposing
    ...
    , getRecipeTasteByID, Normalize(..), normalizeVariants
    , getSimilarRecipes
    ...
    , visualizeRecipeTasteByID, Normalize(..), normalizeVariants
    )
...
normalizeVariants : List Normalize
normalizeVariants =
    [ NormalizeFalse
    , NormalizeTrue
    ]
...
stringFromNormalize : Normalize -> String
stringFromNormalize model =
    case model of
        NormalizeFalse ->
            False
...
computeGlycemicLoad language_query computeGlycemicLoadRequest_body =
    Api.request
        ...
        []
        (Just (Api.Data.encodeComputeGlycemicLoadRequest computeGlycemicLoadRequest_body))
...
-}
getRecipeTasteByID : Int -> Maybe Normalize -> Api.Request Api.Data.GetRecipeTasteByID200Response
getRecipeTasteByID id_path normalize_query =
    Api.request
        "GET"
        "/recipes/{id}/tasteWidget.json"
        [ ( "id", String.fromInt id_path ) ]
        [ ( "normalize", Maybe.map (\val -> if val then "true" else "false")stringFromNormalize normalize_query ) ]
        []
        Nothing
...
{-| Get a recipe's taste. The tastes supported are sweet, salty, sour, bitter, savory, and fatty.
-}
visualizeRecipeTasteByID : Int -> Maybe Normalize -> Maybe String -> Api.Request String
visualizeRecipeTasteByID id_path normalize_query rgb_query =
    Api.request
        "GET"
        "/recipes/{id}/tasteWidget"
        [ ( "id", String.fromInt id_path ) ]
        [ ( "normalize", Maybe.map (\val -> if val then "true" else "false")stringFromNormalize normalize_query ), ( "rgb", Maybe.map identity rgb_query ) ]
        []
        Nothing
        Json.Decode.string
