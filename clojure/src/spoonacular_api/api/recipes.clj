...
(defn-spec get-random-recipes-with-http-info any?
  ...
  ([{:keys [limitLicense includeNutrition include-tags exclude-tags number]} (s/map-of keyword? any?)]
   (call-api "/recipes/random" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limitLicense" limitLicense "includeNutrition" includeNutrition "include-tags" include-tags "exclude-tags" exclude-tags "number" number }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["apiKeyScheme"]})))
