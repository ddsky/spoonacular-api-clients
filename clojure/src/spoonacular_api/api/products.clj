...
(defn-spec get-product-information product-information-spec
  "Get Product Information
  Use a product id to get full information about a product, such as ingredients, nutrition, etc. The nutritional information is per serving."
  [id int?]
  (let [res (:data (get-product-information-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode product-information-spec res st/string-transformer)
       res)))
...
(defn-spec product-nutrition-label-image-with-http-info any?
  ...
  ([id int?, ] (product-nutrition-label-image-with-http-info id nil))
...
(defn-spec product-nutrition-label-image any?
  "Product Nutrition Label Image
  Get a product's nutrition label as an image."
  ([id int?, ] (product-nutrition-label-image id nil))
...
(defn-spec product-nutrition-label-widget-with-http-info any?
  ...
  ([id int?, ] (product-nutrition-label-widget-with-http-info id nil))
...
(defn-spec product-nutrition-label-widget string?
  "Product Nutrition Label Widget
  Get a product's nutrition label as an HTML widget."
  ([id int?, ] (product-nutrition-label-widget id nil))
  ([id int?, optional-params any?]
...
(defn-spec search-grocery-products-with-http-info any?
  ...
  ([query string?, {:keys [minCalories maxCalories minCarbs maxCarbs minProtein maxProtein minFat maxFat addProductInformation offset number]} (s/map-of keyword? any?)]
   (check-required-params query)
