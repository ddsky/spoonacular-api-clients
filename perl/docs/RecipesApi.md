...
eval {
    my $result = $api_instance->get_random_recipes(limit_license => $limit_license, include_nutrition => $include_nutrition, include_tags => $include_tags, exclude_tags => $exclude_tags, number => $number);
    print Dumper($result);
};
if ($@) {
