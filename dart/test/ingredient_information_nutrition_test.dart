//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for IngredientInformationNutrition
void main() {
  // final instance = IngredientInformationNutrition();

  group('test IngredientInformationNutrition', () {
    // Set<SearchGroceryProductsByUPC200ResponseNutritionNutrientsInner> nutrients (default value: const {})
    test('to test the property `nutrients`', () async {
      // TODO
    });

    // Set<IngredientInformationNutritionPropertiesInner> properties (default value: const {})
    test('to test the property `properties`', () async {
      // TODO
    });

    // SearchGroceryProductsByUPC200ResponseNutritionCaloricBreakdown caloricBreakdown
    test('to test the property `caloricBreakdown`', () async {
      // TODO
    });

    // GetShoppingList200ResponseAislesInnerItemsInnerMeasuresOriginal weightPerServing
    test('to test the property `weightPerServing`', () async {
      // TODO
    });


  });

}
