  ...
  class RecipeInformationWinePairing
    ...
    def self.openapi_types
      {
        ...
        :'pairing_text' => :'String',
        :'product_matches' => :'Array<RecipeInformationWinePairingProductMatchesInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::RecipeInformationWinePairing` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::RecipeInformationWinePairing`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@pairing_text.nil? && @pairing_text.to_s.length < 1
        invalid_properties.push('invalid value for "pairing_text", the character length must be great than or equal to 1.')
      end

      if !@product_matches.nil? && @product_matches.length < 0
        invalid_properties.push('invalid value for "product_matches", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@pairing_text.nil? && @pairing_text.to_s.length < 1
      return false if !@product_matches.nil? && @product_matches.length < 0
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] product_matches Value to be assigned
    def product_matches=(product_matches)
      if product_matches.nil?
        fail ArgumentError, 'product_matches cannot be nil'
      end

      if product_matches.length < 0
        fail ArgumentError, 'invalid value for "product_matches", number of items must be greater than or equal to 0.'
      end
