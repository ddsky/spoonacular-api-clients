  ...
  class ImageAnalysisByURL200ResponseNutrition
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::ImageAnalysisByURL200ResponseNutrition` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::ImageAnalysisByURL200ResponseNutrition`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'recipes_used')
        self.recipes_used = attributes[:'recipes_used']
      else
        self.recipes_used = nil
      end
    ...
    def list_invalid_properties
      ...
      if @recipes_used.nil?
        invalid_properties.push('invalid value for "recipes_used", recipes_used cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @recipes_used.nil?
    ...
    def hash
      [recipes_used, calories, fat, protein, carbs].hash
    end
